...
public class AddAlumni implements ActionListener{
	...
	public AddAlumni() {
		...
		prn=new JLabel("Prn :");
		prn.setBounds(250,420,150,30);

		prnField=new JTextField();
		prnField.setBounds(350,420,250,30);
...
//					ResultSet rs=stmt.executeQuery("select aluid from base");
//					while(rs.next())
//					{
...
//					else
//						aluid=year+c;
					stmt.executeUpdate("insert into base values('"+year+"','"+name+"','"+contact+"','"+worksin+"','"+
						worklocation+"','"+designation+"','"+emailid+"','"+prn+" ');");
