...
public class ViewOneAlumni implements ActionListener{
	...
	public void initialize(){
		...
		try{
			...
			ResultSet rs=stmt.executeQuery("select distinct year_of_pass from base");
				while(rs.next())
				{
					k1++;		
		...
		try{
			...
			ResultSet rs=stmt.executeQuery("select distinct year_of_pass from base order by year_of_pass");
				while(rs.next())
				{
		...
		try{
			...
			ResultSet rs=stmt.executeQuery("select distinct work_loc from base");
				while(rs.next())
				{
					k2++;		
		...
		try{
			...
			ResultSet rs=stmt.executeQuery("select distinct work_loc from base order by work_loc");
				while(rs.next())
				{
...
public void actionPerformed(ActionEvent e) {
	...
	try{
		...
		ResultSet rs=stmt.executeQuery("select works from base where works='"+name+"'");
			while(rs.next())
			{
				k++;		
